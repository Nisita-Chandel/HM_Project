// src/pages/KidsProductDetailsPage.jsx
...
export default function KidsProductDetailsPage() {
  const { id } = useParams();
  const dispatch = useDispatch();

  const product = KidsProducts.find((p) => p.id === id);
  ...
  const similarProducts = useMemo(
    () => KidsProducts.filter((p) => p.id !== id).slice(0, 4),
    [id]
  );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="p-10 text-center bg-white rounded-xl shadow">
          ...
          <Link to="/kids" className="underline text-blue-600">
            Go Back to Kids Collection
          </Link>
        </div>
      </div>
    );
