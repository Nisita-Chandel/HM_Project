// src/pages/MenProductDetailsPage.jsx
...
export default function MenProductDetailsPage() {
  ...
  const similarProducts = useMemo(
    () => MenProducts.filter((p) => p.id !== id).slice(0, 4),
    [id]
  );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="p-10 text-center bg-white rounded-xl shadow">
          ...
          <Link to="/men" className="underline text-blue-600">
            Go Back to Men Collection
          </Link>
        </div>
      </div>
    );
      ...
      <div className="flex items-center gap-1">
        {Array.from({ length: total }).map((_, i) => {
          if (i < full) {
            ...
            return <span key={i}>☆</span>; // placeholder for half
          }
          return <span key={i}>☆</span>;
        })}
      </div>
